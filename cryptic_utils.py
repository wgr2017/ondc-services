...
def create_authorisation_header(request_body, created=None, expires=None):
    created = int(round(datetime.datetime.now().timestamp())) if created is None else created
    expires = int(round((datetime.datetime.now() + datetime.timedelta(hours=1)).timestamp())) if expires is None else expires
